...
$txt = "";
		  ...
		  ';
			...
			$ReadTable = $dataBuff->SQL_Select("	
													 select 
														 superaward_status.name as g_name,
                             							 person.id,person.pictype,
	 													 person.name as p_name,
														 person.unit_show
													 from superaward_status
													 left join person
													 on person.id = superaward_status.sid
													 order by  superaward_status.seq
											");	

					while ($row = $ReadTable->fetch_assoc()) 
					{
						$html.='<tr><td width="40" align="right">'.++$i.'&nbsp;&nbsp;</td><td width="300">&nbsp;'.$row['g_name'].'</td><td>&nbsp;'.$row['p_name'].'</td><td>&nbsp;'.$row['unit_show'].'</td>'.'</tr>';
					}
