<?	
	...
	$dataBuff = new Cl_award();
		...
		if($mode === "readData")
			{
				...
				$ReadTable = $dataBuff->SQL_Select("select * from award_pick_number");	
					if($ReadTable)
					{
		...
		if($mode === "addData")
			{			
				$dataBuff->SQL_Query(" INSERT INTO unit_class (unit, c1a, c1b, c1c, c2a, c2b, c2c, c3a, c3b, c3c )
										VALUES ('$unit_name', $c1a, $c1b, $c1c, $c2a, $c2b, $c2c, $c3a, $c3b, $c3c )
									");
			}
		...
		if($mode === "deleteData")
			{			
				...
				$dataBuff->SQL_Query(" DELETE FROM unit_class WHERE id = $id");
	
			}
			
		if($mode === "ReadUpdateData")
			{			
				$ReadInput = $dataBuff->SQL_Select("select * from unit_class where id = $id");
				if($ReadInput)
					{
						$ans = array(array("result" => "true", "msg" => "success"));
		...
		if($mode === "UpdateData")
			{			
				$dataBuff->SQL_Query(" 	UPDATE unit_class
										SET unit = '$unit_name', c1a = $c1a, c1b = $c1b, c1c = $c1c, c2a = $c2a, c2b = $c2b, c2c = $c2c, c3a = $c3a, c3b = $c3b, c3c = $c3c  
										WHERE id = $id;
									");
		...
		if($mode === "readData_gift")
			{
				...
				$ReadTable = $dataBuff->SQL_Select("select * from gift_little");	
					if($ReadTable)
					{
						$ans = array(array("result" => "true", "msg" => "success"));
							while ($row = $ReadTable->fetch_assoc()) 
		...
		if($mode === "AddData_gift")
			{
					...
					$dataBuff->SQL_Query(" INSERT INTO gift_little (name, type, status )
		...
		if($mode === "Gift_deleteData")
			{			
				...
				$dataBuff->SQL_Query(" DELETE FROM gift_little WHERE id = $id");
	
			}
		
		if($mode === "Gift_ReadUpdateData")
			{			
				$ReadInput = $dataBuff->SQL_Select("select * from gift_little where id = $id");
				if($ReadInput)
					{
						$ans = array(array("result" => "true", "msg" => "success"));
							while ($row = $ReadInput->fetch_assoc()) 
		...
		if($mode === "Gift_UpdateData")
			{		
				...
				$dataBuff->SQL_Query(" 	UPDATE gift_little
